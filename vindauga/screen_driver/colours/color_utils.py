...
def create_attribute(fg_color: int | str | ColourRGB = None, bg_color: int | str | ColourRGB = None,
    ...
    def parse_color(color) -> DesiredColour:
        if color is None:
            return DesiredColour()  # Default
        elif isinstance(color, int):
            if 0 <= color <= 15:
                return DesiredColour.from_bios(ColourBIOS(color))
            elif 0 <= color <= 255:
                return DesiredColour.from_xterm(ColourXTerm(color))
            else:
        ...
        elif isinstance(color, ColourRGB):
            return DesiredColour.from_rgb(color)
        else:
...
def get_color_distance(color1: ColourRGB, color2: ColourRGB) -> float:
    """
...
def find_best_palette_match(rgb: ColourRGB, palette_size: int = 16) -> int:
    """
    Find the best matching color index in a palette.
...
def create_gradient(start_rgb: ColourRGB, end_rgb: ColourRGB, steps: int) -> list[ColourRGB]:
    ...
    :param start_rgb: RGB color to start from.
    :param end_rgb: RGB color to end from.
...
def blend_colors(color1: ColourRGB, color2: ColourRGB, alpha: float) -> ColourRGB:
    """
    Blend two RGB colors with given alpha (0.0 = color1, 1.0 = color2).

    :param color1: RGB color to blend.
    :param color2: RGB color to blend.
